...
import { IClubCreationRequest, IClubJoiningRequest } from "../Interfaces/ClubsIntefaces";
import api from "./API";
...
export async function CreateClubAPI(form:IClubCreationRequest){
  try{
      await axios.post("https://localhost:7048/api/clubs/request-creating/",form,{
        headers:{Authorization:`bearer ${localStorage.getItem("authToken")}`}
      });    //await api.post(`clubs/request-creating/`,form,{
...
export async function CheckClubRequestCreationAPI(){
  try{
      const res=await axios.get(`https://localhost:7048/api/clubs/check-requested-club-creation/`,{
        headers:{Authorization:`bearer ${localStorage.getItem("authToken")}`}
      });
