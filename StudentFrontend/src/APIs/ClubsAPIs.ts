...
export async function GetEventMembersAPI(eventId:number){

     return (await api.get(`clubs/events/members${eventId}`)).data.data;

    
}

export async function CreateClubAPI(form:FormData){
  try{
...
export async function CheckClubRequestCreationAPI(){
  try{
      const res=await api.get(`clubs/check-requested-club-creation/`,{
        headers:{Authorization:`bearer ${localStorage.getItem("authToken")}`}
      });
